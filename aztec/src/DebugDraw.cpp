...
DebugDrawCamera g_DebugDrawCamera;

//
b2Vec2 DebugDrawCamera::ConvertScreenToWorld(const b2Vec2 &ps)
{
...
//
b2Vec2 DebugDrawCamera::ConvertWorldToScreen(const b2Vec2 &pw)
{
  float32 w     = float32(m_width);
  float32 h     = float32(m_height);
  float32 ratio = w / h;
...
void DebugDrawCamera::BuildProjectionMatrix(float32 *m, float32 zBias)
{
  float32 w     = float32(m_width);
  float32 h     = float32(m_height);
  float32 ratio = w / h;
  b2Vec2  extents(ratio * 25.0f, ratio * 25.0f);
  extents *= m_zoom;
...
struct GLRenderPoints {
  ...
  void Flush()
  {
    ...
    //g_DebugDrawCamera.BuildProjectionMatrix(proj, 0.0f);

    //glUniformMatrix4fv(m_projectionUniform, 1, GL_FALSE, proj);
    glUniformMatrix4fv(m_projectionUniform, 1, GL_FALSE, glm::value_ptr(GameEngine::getInstance()->getCurrentGameState()->getActiveCamera()->getDebugDrawProjectionMatrix()));
...
struct GLRenderLines {
  ...
  void Vertex(const b2Vec2 &v, const b2Color &c)
  {
    ...
    m_vertices[m_count] = v;
    m_colors[m_count]   = b2Color(0, 1, 0, 0.25f); // c;
    ++m_count;
  }
  ...
  void Flush()
  {
    ...
    float32 proj[16] = {0.0f};
    //g_DebugDrawCamera.BuildProjectionMatrix(proj, 0.1f);
...
struct GLRenderTriangles {
  ...
  void Vertex(const b2Vec2 &v, const b2Color &c)
  {
    ...
    m_vertices[m_count] = v;
    m_colors[m_count]   = b2Color(0, 1.0f, 0, 1.0f); // c;
    ++m_count;
  }
  ...
  void Flush()
  {
    ...
    float32 proj[16] = {0.0f};
    //g_DebugDrawCamera.BuildProjectionMatrix(proj, 0.2f);
